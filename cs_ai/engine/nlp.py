...
from paths import chroma_db_path, resolve_company_file
...
# English-specific markers used for mixed-language detection.
# These words appear in English B2B emails and are unlikely to be written
# by a native French speaker in a purely French message.
_ENGLISH_MARKERS = [
    # Articles / function words — unambiguous EN
    "the", "is", "are", "have", "has", "was", "were",
    # Business email vocabulary
    "shipment", "tracking", "invoice", "refund", "unfortunately",
    "however", "regarding", "attached", "kindly", "sincerely",
    "please", "delivery", "package", "payment", "purchase",
    "order", "complaint", "receipt", "warehouse", "supplier",
    "dispatch", "carrier", "freight", "parcel", "billing",
    # Closing / greeting
    "yours", "regards", "dear", "hello", "hi",
]

_WEIGHT_MAP  = {"critical": 3, "strong": 2, "moderate": 1}
_CHROMA_PATH = chroma_db_path()
...
class NLPEngine:
    ...
    def _try_build_kb_from_file(self, kb_path: str | None = None):
        """Loads knowledge_base.json and builds KB embeddings if the file exists."""
        import json as _json
        if kb_path is None:
            kb_path = resolve_company_file("knowledge_base.json")
    ...
    def detect_language(self, text: str) -> tuple:
        """
        Returns (language: str, confidence: float, is_mixed: bool).

        Confidence tiers
        ----------------
        French >= 5 keyword matches  → 0.95
        French  2–4 keyword matches  → 0.70
        French  1   keyword match    → 0.50
        No French matches            → "English", 0.90, False
        Roughly equal FR + EN counts → dominant language, 0.55, True
        """
        fr_count = sum(
        ...
        en_count = sum(
            1 for w in _ENGLISH_MARKERS
            if re.search(r'\b' + re.escape(w) + r'\b', text)
        )

        if fr_count == 0:
            return "English", 0.90, False

        # Mixed: both languages present and neither dominates by more than 2×
        is_mixed = (
            en_count >= 2
            and fr_count >= 2
            and min(fr_count, en_count) >= max(fr_count, en_count) * 0.40
        )

        language = "French" if fr_count >= en_count else "English"
...
# ==============================================================================
# NOISE DETECTION — purely rule-based, no embeddings required
# ==============================================================================

def detect_noise(text: str, subject: str = "", sender: str = "") -> dict:
    """
    Detect machine-generated emails that should bypass AI processing.

    Design principles
    -----------------
    1. Only emails where the ENTIRE message is machine-generated are flagged.
       A human mentioning absence as context ("I was on holiday when the parcel
       arrived damaged") is NOT flagged.

    2. Two-tier OOO strategy:
       - Subject match  : short/ambiguous phrases are safe here — a human would
                          never title their own email "Out of office".
       - Body match     : regex patterns that capture the *semantic field* of
                          automated absence declarations (first-person statements
                          announcing unavailability + return/alternative-contact).
                          Short fragments like "on holiday" are NOT body-matched.

    3. Regex is used throughout for broader coverage — the goal is to capture
       the *concept* (automated system announcing someone is unavailable), not
       just a specific list of hardcoded strings.
    ...
    send = sender.strip().lower()

    def _match_any_literal(text_to_check: str, phrases: tuple) -> str | None:
        for p in phrases:
            if p in text_to_check:
                return p
        return None

    def _match_any_regex(text_to_check: str, patterns: tuple) -> str | None:
        for pat in patterns:
    ...
    # ── Auto-reply — subject prefix ───────────────────────────────────────────
    # Mail clients prepend a standard tag; no human titles their own email this way.
    _auto_subj_prefixes = (
        "auto:",
        "automatic reply",
        "automatic response",
    ...
    hit = _match_any_literal(subj, _auto_subj_prefixes)
    if hit and subj.startswith(hit):
        return {"is_noise": True, "noise_type": "auto_reply",
                "reason": f"Subject starts with '{hit}'"}

    # ── Auto-reply — no-reply sender ─────────────────────────────────────────
    _noreply_re = re.compile(
        r'(noreply|no[-_]reply|donotreply|do[-_]not[-_]reply'
        r'|ne[-_]pas[-_]repondre|nepasrepondre)@',
        re.IGNORECASE,
    )
    m = _noreply_re.search(send)
    if m:
        return {"is_noise": True, "noise_type": "auto_reply",
                "reason": f"Sender is a no-reply address ('{m.group(0)}')"}

    # ── Auto-reply — body: explicit machine-generated declarations ────────────
    # Literal phrases that only automated systems write.
    _auto_body_literals = (
        ...
        "cette réponse est automatique",
        "ce message a été envoyé automatiquement",
        "ne répondez pas à cet e-mail",
        "ne répondez pas à ce message",
        "cette boîte mail n'est pas surveillée",
        "cette adresse ne reçoit pas les réponses",
    )
    hit = _match_any_literal(body, _auto_body_literals)
    if hit:
        return {"is_noise": True, "noise_type": "auto_reply",
                "reason": f"Body contains automated declaration '{hit}'"}

    # Auto-reply: system acknowledgment patterns (ticketing, CRM, portals).
    # "Thank you for contacting us. Your request / ticket / case has been…"
    _auto_body_re = (
        # System acknowledgment: "thank you for contacting / your request has been received"
        r'thank you for (contacting|reaching out|your (email|message|inquiry))'
        r'.{0,120}(will (be (in touch|processed|handled|responded)|respond|contact you))',
        r'(your (request|ticket|case|inquiry|message|email))\s+(has been|have been)'
        r'\s+(received|logged|registered|created|opened|assigned)',
        r'(we (have received|received) your (request|email|message|inquiry))',
        r'(référence|ticket|numéro de dossier|case id|request id)[^\n]{0,30}#?\s*\d+',
        r'merci (de nous avoir contactés|pour votre (message|demande|email))'
        r'.{0,120}(sera (traité|pris en charge|répondu)|vous (contacterons|répondrons))',
    )
    hit = _match_any_regex(body, _auto_body_re)
    if hit:
        return {"is_noise": True, "noise_type": "auto_reply",
                "reason": f"Body matches system-acknowledgment pattern ('{hit[:60]}…')"}

    # ── Out of office — subject (short phrases, subject only) ────────────────
    # These labels appear in subject lines of automated OOO messages.
    # A human titling their own email "Out of office" or "Absent du bureau"
    ...
    _ooo_subj_literals = (
        ...
        "vacation reply",
        "holiday notice",
        "holiday autoresponse",
        "congé",          # short but safe in subject context
        "en vacances",
        "on holiday",
        "on vacation",
    )
    hit = _match_any_literal(subj, _ooo_subj_literals)
    if hit:
        return {"is_noise": True, "noise_type": "out_of_office",
                "reason": f"Subject contains '{hit}'"}

    # ── Out of office — body (semantic regex, complete sentences only) ────────
    # Captures the semantic field of "I am unavailable until <date/event>".
    # Patterns cover: OOO, holidays, sick leave, parental/maternity leave,
    # sabbatical, business travel, conference, training — any reason the sender
    # is giving for their absence in a machine-sent auto-reply.
    # NOT matched: short fragments in long emails (a customer saying
    # "I was on sick leave when the order arrived" won't trigger this).
    _ooo_body_re = (
        # First-person present-state unavailability
        r"i (am|'m) (currently\s+)?(out of (the )?office|away from (the )?office"
        r"|on (holiday|vacation|leave|sick leave|maternity leave|paternity leave"
        r"|parental leave|sabbatical|annual leave|compassionate leave)"
        r"|absent|unavailable|unreachable|travelling|traveling|at a conference"
        r"|at a training)",

        # First-person future return
        r"i (will|'ll) (be )?(back|return|respond|reply|be available|be in (the )?office)"
        r"\s+(on|from|after|the week of|starting)",

        # First-person limited access
        r"i (will|'ll) have (limited|no|reduced) (access to (email|my (email|inbox|messages))"
        r"|email access)",

        # Colleague/delegate pattern
        r"(my colleague|my assistant|a colleague|our team)\s+.{0,60}"
        r"(will (handle|take care of|respond|assist|be your contact))",

        # Urgent-contact redirect (strong OOO signal)
        r"for (urgent (matters|requests|assistance|inquiries)|immediate (assistance|help))"
        r",?\s+please (contact|reach|email|call)",

        # French equivalents
        r"je (suis|serai) (actuellement\s+)?(absent(e)?( du bureau)?|en (congé|vacances"
        r"|arrêt (maladie|de travail)|congé (maternité|paternité|parental)|déplacement"
        r"|formation|séminaire|conférence)|indisponible|injoignable)",

        r"je (serai de retour|reviendrai|répondrai) (le|à partir du|dès le|la semaine)",

        r"(mon|ma) (collègue|assistante?|remplaçant(e)?)\s+.{0,60}"
        r"(prendra|gérera|s'occupera|vous (contactera|répondra))",

        r"pour (toute urgence|les urgences|toute demande urgente),?\s+"
        r"(veuillez|merci de) (contacter|joindre|appeler)",
    )
    hit = _match_any_regex(body, _ooo_body_re)
    if hit:
        return {"is_noise": True, "noise_type": "out_of_office",
                "reason": f"Body matches absence declaration ('{hit[:60]}…')"}
    ...
    hit = _match_any_literal(subj, _fail_subj_literals)
    if hit:
        return {"is_noise": True, "noise_type": "delivery_failure",
                "reason": f"Subject contains '{hit}'"}

    _fail_sender_re = re.compile(
        r'(mailer[-_]?daemon|postmaster|mail[-_]?daemon|mailerdaemon)@',
        re.IGNORECASE,
    )
    m = _fail_sender_re.search(send)
    if m:
        return {"is_noise": True, "noise_type": "delivery_failure",
                "reason": f"Sender is a bounce address ('{m.group(0)}')"}

    # ── Spam — URL flood ──────────────────────────────────────────────────────
    # > 5 URLs is a reliable spam signal.
    # Word-count alone is NOT used: "Where is my order?" (5 words) is legitimate.
    url_count = len(re.findall(r'https?://\S+', text, re.IGNORECASE))
    if url_count > 5:
        return {"is_noise": True, "noise_type": "spam",
                "reason": f"Body contains {url_count} URLs (threshold: 5)"}

    return {"is_noise": False, "noise_type": None, "reason": ""}
...
if __name__ == "__main__":
    ...
    engine._try_build_kb_from_file()  # resolves via paths.resolve_company_file
    print("Done.")
