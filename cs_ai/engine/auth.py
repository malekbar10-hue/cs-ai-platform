...
class AuthManager:
    ...
    @staticmethod
    def get_env_var(var_name: str, company: str = None) -> str:
        """
        Read a secret from the environment, trying the company-prefixed name first.

        Lookup order (example: company="alpha", var_name="ERP_TOKEN"):
            1. ALPHA_ERP_TOKEN
            2. ERP_TOKEN
        ...
        ----------
        var_name : str
            The bare variable name (e.g. "ERP_TOKEN").
        company  : str | None
            Company identifier.  Defaults to CS_AI_COMPANY env var.

        Raises
        ------
        MissingEnvVarError
            If neither the prefixed nor the bare variable is set.
        """
        company = company or os.environ.get("CS_AI_COMPANY", "")
        prefixed = f"{company.upper()}_{var_name}" if company else var_name
        value = os.environ.get(prefixed) or os.environ.get(var_name)
        if not value:
            raise MissingEnvVarError(
                f"Missing secret: set {prefixed} (or {var_name}) in your .env file."
            )
        return value
    ...
    def validate_env_vars(auth_config: dict) -> list[str]:
        ...
        Names are reported as their company-prefixed form (e.g. ALPHA_ERP_TOKEN)
        so the error message tells the user exactly what to add.
        ...
        for config_key in required_keys:
            ...
            if not env_var_name:
                ...
                continue
            try:
                AuthManager.get_env_var(env_var_name, company)
            except MissingEnvVarError:
                prefixed = f"{company.upper()}_{env_var_name}" if company else env_var_name
    ...
    def _read_env(auth_config: dict, config_key: str) -> str:
        ...
        respecting the company prefix (CS_AI_COMPANY env var).

        Example:
            CS_AI_COMPANY=alpha, auth_config = {"token_env_var": "ERP_TOKEN"}
            _read_env(auth_config, "token_env_var")  →  os.environ["ALPHA_ERP_TOKEN"]
        """
        env_var_name = auth_config.get(config_key, "")
        if not env_var_name:
        ...
        return AuthManager.get_env_var(env_var_name)
